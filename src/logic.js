...
var tla = 'MAR'
console.log(wc.reduce((p, c) => {
    const homeScore0 = c.score.fullTime.home;
    const awayScore0 = c.score.fullTime.away;
    const homeRegular = c.score.regularTime?.home;
    const homeXtra = c.score.extraTime?.home;
    const homePenalties = c.score.penalties?.home;
    const awayRegular = c.score.regularTime?.away;
    const awayXtra = c.score.extraTime?.away;
    const awayPenalties = c.score.penalties?.away;
    const homeScore1 = homeRegular == null ? homeScore0 : homeRegular + homeXtra + (homePenalties ?? 0);
    const awayScore1 = awayRegular == null ? awayScore0 : awayRegular + awayXtra + (awayPenalties ?? 0);
    const cs0 = c.homeTeam.tla == tla ? homeScore0 : c.awayTeam.tla == tla ? awayScore0 : 0;
    const cs1 = c.homeTeam.tla == tla ? homeScore1 : c.awayTeam.tla == tla ? awayScore1 : 0;
    if (cs0 !== cs1) {
        console.log(c.id + ' ' + cs0 + ' ' + cs1);
    }
    return p + cs0;
}, 0,
),
